...
async function seedTwo(nonce, prefix) {
...
describe('tenants module — RLS isolation via withTenantContext', () => {
    ...
    it('ctx=A can read tenant A but not tenant B', async () => {
        ...
        const { rowA, rowB } = await runWithTenant({ tenantId: idA, userId: 'u', role: 'ADMIN' }, async () => {
            const scoped = prisma.$extends(withTenantContext({ tenantId: idA, userId: 'u', role: 'ADMIN' }));
            return {
    ...
    it('ctx=A cannot update tenant B (RLS hides row, Prisma throws P2025)', async () => {
        ...
        const { updateError, bName } = await runWithTenant({ tenantId: idA, userId: 'u', role: 'ADMIN' }, async () => {
            const scoped = prisma.$extends(withTenantContext({ tenantId: idA, userId: 'u', role: 'ADMIN' }));
            ...
            return { updateError, bName: undefined };
        });
        expect(updateError).not.toBeNull();
        expect(String(updateError)).toMatch(/not found/i);
        // Verify tenant B's name did not change by reading it under ctx=B.
        const verifiedB = await runWithTenant({ tenantId: idB, userId: 'u', role: 'ADMIN' }, async () => {
            const scoped = prisma.$extends(withTenantContext({ tenantId: idB, userId: 'u', role: 'ADMIN' }));
