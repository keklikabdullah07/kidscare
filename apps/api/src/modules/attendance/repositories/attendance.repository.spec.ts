...
const mockPrismaAttendance: PrismaAttendance = {
  id: 'att-1',
  tenantId: 't-1',
  studentId: 's-1',
...
describe('AttendanceRepository', () => {
  ...
  it('upsert delegates to prisma upsert with create & update data', async () => {
    ...
    withTenant.mockImplementation((fn: (c: unknown) => unknown) => fn({ attendance: { upsert } }));
    const date = new Date('2026-09-15');
