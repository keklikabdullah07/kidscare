...
describe('StudentsRepository', () => {
    ...
    describe('findMany', () => {
        it('returns rows for tenant, excluding soft-deleted by default', async () => {
            withTenant.mockImplementation((fn) => fn({ student: { findMany: jest.fn().mockResolvedValue([mockStudent]) } }));
    ...
    describe('findById', () => {
        it('returns the student when found', async () => {
            withTenant.mockImplementation((fn) => fn({ student: { findFirst: jest.fn().mockResolvedValue(mockStudent) } }));
            await expect(repo.findById('t-1', 's-1')).resolves.toEqual(mockStudent);
        });
        it('returns null when not found', async () => {
            withTenant.mockImplementation((fn) => fn({ student: { findFirst: jest.fn().mockResolvedValue(null) } }));
            await expect(repo.findById('t-1', 'missing')).resolves.toBeNull();
        });
    });
    describe('insert', () => {
        it('creates a student under the given tenant', async () => {
            withTenant.mockImplementation((fn) => fn({
                student: {
    ...
    describe('update', () => {
        it('updates the student', async () => {
            ...
            withTenant.mockImplementation((fn) => fn({ student: { update: jest.fn().mockResolvedValue(updated) } }));
