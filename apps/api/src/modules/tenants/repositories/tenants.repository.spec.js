...
describe('TenantsRepository', () => {
    ...
    describe('findById', () => {
        it('returns the tenant when found', async () => {
            withTenant.mockImplementation((fn) => fn({ tenant: { findUnique: jest.fn().mockResolvedValue(mockTenant) } }));
            await expect(repo.findById('t-1')).resolves.toEqual(mockTenant);
            expect(withTenant).toHaveBeenCalledTimes(1);
        });
        it('returns null when missing', async () => {
            withTenant.mockImplementation((fn) => fn({ tenant: { findUnique: jest.fn().mockResolvedValue(null) } }));
    ...
    describe('update', () => {
        it('returns the updated tenant', async () => {
            ...
            withTenant.mockImplementation((fn) => fn({ tenant: { update: jest.fn().mockResolvedValue(updated) } }));
