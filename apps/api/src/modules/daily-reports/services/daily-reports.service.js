...
let DailyReportsService = (() => {
    let _classDecorators = [Injectable()];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    var DailyReportsService = class {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            DailyReportsService = _classThis = _classDescriptor.value;
        ...
        async saveReport(tenantId, studentId, dateStr, input) {
            const date = new Date(dateStr);
            const updateData = {};
            ...
            if (input.naps !== undefined)
                updateData.naps = input.naps;
            if (input.potty !== undefined)
                updateData.potty = input.potty;
            if (input.activities !== undefined)
                updateData.activities = input.activities;
            if (input.medications !== undefined)
                updateData.medications = input.medications;
            if (input.teacherNote !== undefined)
                updateData.teacherNote = input.teacherNote;
            const saved = await this.repo.upsert(tenantId, studentId, date, updateData);
            return DailyReport.fromPrisma(saved);
        }
    };
    return DailyReportsService = _classThis;
})();
export { DailyReportsService };
