...
describe('ParentController', () => {
    ...
    describe('getChildren', () => {
        it('calls service with current user and returns children overviews', async () => {
            ...
            const res = await controller.getChildren({ userId: 'u-1', tenantId: 't-1', role: 'PARENT' }, '2026-09-15');
            expect(res).toHaveLength(1);
            expect(res[0]?.student.firstName).toBe('Ada');
            expect(service.getChildrenOverview).toHaveBeenCalledWith('t-1', 'u-1', 'PARENT', '2026-09-15');
        });
    });
});
