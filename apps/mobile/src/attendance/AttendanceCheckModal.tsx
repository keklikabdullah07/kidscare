...
export function AttendanceCheckModal({
  ...
  async function handleQuickCheckIn(): Promise<void> {
  ...
  async function handleStatusChange(newStatus: AttendanceStatus): Promise<void> {
    if (!student || saving) return;
    setSaving(true);
    try {
  ...
  const isCustomPickup =
    selectedContactId === 'CUSTOM' || (!selectedContactId && authorizedContacts.length === 0);
            ...
            {loading ? (
              ...
              <View style={styles.content}>
                ...
                <View style={[styles.statusBox, { backgroundColor: statusInfo.bg }]}>
                  ...
                  {attendance?.pickupNote && (
                    <Text style={styles.noteText}>Not: {attendance.pickupNote}</Text>
                  )}
                </View>
                ...
                {!showCheckOutForm && (
                  <View style={styles.actionGrid}>
                    {currentStatus !== 'PRESENT' && currentStatus !== 'LEFT' && (
                    ...
                    <View style={styles.statusBtnRow}>
                      {currentStatus !== 'EXCUSED' && (
                        <Pressable
                          style={[styles.statusBtn, styles.excusedBtn]}
                          onPress={() => void handleStatusChange('EXCUSED')}
                          disabled={saving}
                        >
                      ...
                      {currentStatus !== 'ABSENT' && (
                        <Pressable
                          style={[styles.statusBtn, styles.absentBtn]}
                          onPress={() => void handleStatusChange('ABSENT')}
                          disabled={saving}
                        >
                ...
                {showCheckOutForm && (
                  <View style={styles.checkOutForm}>
                      ...
                      authorizedContacts.map((c, idx) => {
                        ...
                        return (
                          <Pressable
                            ...
                            onPress={() => setSelectedContactId(contactKey)}
                          >
                    ...
                    {isCustomPickup && (
                      <View style={styles.customBox}>
                        ...
                        <TextInput
                          ...
                          placeholder="Örn: Anne telefonla arayarak onay verdi."
                          multiline
                        />
                      </View>
                    )}
                    ...
                    <View style={styles.checkoutActions}>
                      ...
                      <Pressable
                        style={[styles.mainBtn, styles.checkOutBtn, { flex: 1 }]}
                        onPress={() => void handleCompleteCheckOut()}
                        disabled={saving}
                      >
