...
export function ActivityGalleryModal({ visible, onClose, userRole, classroom, }) {
    ...
    return (<Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.header}>
          ...
          {canPost ? (<TouchableOpacity style={styles.addBtn} onPress={() => setCreateModalVisible(true)}>
        ...
        <View style={styles.filterBar}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterScroll}>
            ...
            return (<TouchableOpacity key={tag} style={[styles.filterChip, active && styles.filterChipActive]} onPress={() => setSelectedTag(tag)}>
        ...
        {/* Posts Content */}
        {loading && !refreshing ? (<View style={styles.centerBox}>
            <ActivityIndicator size="large" color="#4F46E5"/>
            <Text style={styles.loadingText}>Etkinlik fotoğrafları yükleniyor…</Text>
          </View>) : (<ScrollView contentContainerStyle={styles.scrollContent} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => {
                    setRefreshing(true);
                    void loadPosts();
                }}/>}>
            {posts.length === 0 ? (<View style={styles.emptyCard}>
              ...
              </View>) : (posts.map((post) => (<ActivityCard key={post.id} post={post} canDelete={canPost} onDelete={(id) => {
                    void handleDelete(id);
                }}/>)))}
          </ScrollView>)}

        {/* Create Activity Modal */}
        <CreateActivityModal visible={createModalVisible} onClose={() => setCreateModalVisible(false)} defaultClassroom={classroom} onCreated={(newPost) => {
