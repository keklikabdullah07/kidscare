...
export function DailyReportModal({ student, date, visible, onClose, onSaved, }) {
          ...
          {loading ? (<View style={styles.center}>
            ...
            <View style={styles.sectionContainer}>
                  ...
                  <View style={styles.card}>
                    ...
                    {pottyEntries.length > 0 ? (<View style={styles.tagWrap}>
                        {pottyEntries.map((p) => {
                        const info = POTTY_TYPES.find((pt) => pt.key === p.type);
                        return (<View key={p.id} style={styles.pill}>
                              <Text style={styles.pillText}>
                                {info?.emoji} {p.time} ({info?.label})
                              </Text>
                            </View>);
                    })}
                      </View>) : (<Text style={styles.mutedText}>Kayıt bulunmuyor.</Text>)}
                  </View>
                  ...
                  <View style={styles.card}>
                    ...
                    {activities.length > 0 ? (<View style={styles.tagWrap}>
                        {activities.map((a) => (<View key={a} style={styles.activityPill}>
                            <Text style={styles.activityPillText}>✓ {a}</Text>
                          </View>))}
                      </View>) : (<Text style={styles.mutedText}>Etkinlik seçilmedi.</Text>)}
                  </View>

                  {/* Teacher Note */}
                  {teacherNote ? (<View style={styles.card}>
            ...
            <View style={styles.sectionContainer}>
                  ...
                  <View style={styles.moodGrid}>
                    ...
                    return (<Pressable key={m.key} style={[styles.moodBtn, sel && styles.moodBtnActive]} onPress={() => setMood(m.key)}>
                  ...
                  <View style={styles.tagWrap}>
                    {MEAL_PORTIONS.map((mp) => (<Pressable key={mp.key} style={[styles.choiceBtn, breakfast === mp.key && styles.choiceBtnActive]} onPress={() => setBreakfast(mp.key)}>
                        <Text style={[
                  ...
                  <View style={styles.tagWrap}>
                    {MEAL_PORTIONS.map((mp) => (<Pressable key={mp.key} style={[styles.choiceBtn, lunch === mp.key && styles.choiceBtnActive]} onPress={() => setLunch(mp.key)}>
                        <Text style={[
                        styles.choiceBtnText,
                  ...
                  <View style={styles.tagWrap}>
                    {MEAL_PORTIONS.map((mp) => (<Pressable key={mp.key} style={[styles.choiceBtn, snack === mp.key && styles.choiceBtnActive]} onPress={() => setSnack(mp.key)}>
                        <Text style={[
                        styles.choiceBtnText,
                  ...
                  <TextInput style={styles.textInput} value={napStart} onChangeText={setNapStart} placeholder="13:00"/>
                  <Text style={styles.fieldLabel}>Uyanma Saati (SS:DD)</Text>
                  <TextInput style={styles.textInput} value={napEnd} onChangeText={setNapEnd} placeholder="14:30"/>
                  <Text style={styles.fieldLabel}>Uyku Kalitesi</Text>
                  <View style={styles.tagWrap}>
                    {NAP_QUALITIES.map((nq) => (<Pressable key={nq.key} style={[styles.choiceBtn, napQuality === nq.key && styles.choiceBtnActive]} onPress={() => setNapQuality(nq.key)}>
                        <Text style={[
                        styles.choiceBtnText,
                  ...
                  <View style={styles.pottyAddBox}>
                    <TextInput style={styles.textInput} value={pottyTime} onChangeText={setPottyTime} placeholder="Saat (Örn: 10:30)"/>
                    <View style={styles.tagWrap}>
                      {POTTY_TYPES.map((pt) => (<Pressable key={pt.key} style={[styles.choiceBtn, pottyType === pt.key && styles.choiceBtnActive]} onPress={() => setPottyType(pt.key)}>
                          <Text style={[
                        styles.choiceBtnText,
                  ...
                  {pottyEntries.map((p) => {
                    const info = POTTY_TYPES.find((pt) => pt.key === p.type);
                    return (<View key={p.id} style={styles.pottyRow}>
                  ...
                  <View style={styles.tagWrap}>
                    ...
                    return (<Pressable key={qa} style={[styles.choiceBtn, sel && styles.activityBtnActive]} onPress={() => toggleActivity(qa)}>
                  ...
                  <TextInput style={[styles.textInput, styles.multiline]} value={teacherNote} onChangeText={setTeacherNote} multiline numberOfLines={3} placeholder="Günün nasıl geçtiği hakkında veliye özel not..."/>
