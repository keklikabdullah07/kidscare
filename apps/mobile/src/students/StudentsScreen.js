...
export function StudentsScreen() {
    ...
    return (<View style={styles.container}>
      ...
      {errorMsg && (<Text style={styles.error} role="alert">
          {errorMsg}
        </Text>)}
      ...
      {status === 'ready' && students.length === 0 && (<Text style={styles.empty}>Henüz öğrenci yok. Yukarıdan ekleyin.</Text>)}
      {status === 'ready' && students.length > 0 && (<FlatList data={students} keyExtractor={(item) => item.id} renderItem={({ item }) => {
                const att = attendanceMap[item.id];
                const attStatus = att?.status ?? 'ABSENT';
                const attInfo = ATTENDANCE_BADGES[attStatus];
                return (<Pressable style={styles.row} onLongPress={() => handleDelete(item)} onPress={() => setTrackingStudent(item)}>
                <View style={styles.flex1}>
                  <View style={styles.nameRow}>
                    <Text style={styles.rowName}>
                      {item.firstName} {item.lastName}
                    </Text>
                    {item.passport?.bloodType && item.passport.bloodType !== 'UNKNOWN' && (<View style={styles.bloodPill}>
                        <Text style={styles.bloodPillText}>🩸 {item.passport.bloodType}</Text>
                      </View>)}
                    {item.passport?.allergies && item.passport.allergies.length > 0 && (<View style={styles.allergyPill}>
                  ...
                  {item.notes && (<Text style={styles.rowNotes} numberOfLines={2}>
                      {item.notes}
                    </Text>)}
                </View>
                <View style={styles.rightCol}>
                  ...
                  <View style={styles.actionBtnRow}>
                    <Pressable style={styles.attendanceBtn} onPress={() => setAttendanceStudent(item)}>
      ...
      <NewStudentModal visible={modalOpen} saving={saving} onClose={() => setModalOpen(false)} onSaved={(s) => {
            setStudents((prev) => [...prev, s].sort(byLastName));
            setModalOpen(false);
        }} onError={setErrorMsg} onSavingChange={setSaving}/>

      <StudentPassportModal student={passportStudent} visible={passportStudent !== null} onClose={() => setPassportStudent(null)} onSaved={(updatedPassport) => {
            if (passportStudent) {
                const updated = { ...passportStudent, passport: updatedPassport };
                setStudents((prev) => prev.map((s) => (s.id === updated.id ? updated : s)));
                setPassportStudent(updated);
            }
        }}/>

      <DailyReportModal student={trackingStudent} date={todayStr} visible={trackingStudent !== null} onClose={() => setTrackingStudent(null)}/>

      <AttendanceCheckModal student={attendanceStudent} date={todayStr} visible={attendanceStudent !== null} onClose={() => setAttendanceStudent(null)} onAttendanceUpdated={(att) => {
            setAttendanceMap((prev) => ({ ...prev, [att.studentId]: att }));
        }}/>

      <DailyMenuModal date={todayStr} visible={menuModalOpen} onClose={() => setMenuModalOpen(false)}/>

      <ActivityGalleryModal visible={galleryModalOpen} onClose={() => setGalleryModalOpen(false)} userRole={state.status === 'authenticated' ? state.user.role : undefined}/>
    </View>);
}
...
function NewStudentModal({ visible, saving, onClose, onSaved, onError, onSavingChange, }) {
      ...
      <View style={styles.modalBackdrop}>
        <View style={styles.modalSheet}>
          <ScrollView keyboardShouldPersistTaps="handled">
            ...
            <Field label="Doğum tarihi (YYYY-AA-GG)" value={dateOfBirth} onChange={setDateOfBirth}/>
            ...
            <View style={styles.modalActions}>
              ...
              <Pressable style={[styles.modalButton, styles.modalButtonPrimary]} onPress={() => void handleSave()} disabled={saving}>
...
function Field({ label, value, onChange, multiline = false, }) {
    return (<View style={styles.field}>
      ...
      <TextInput style={[styles.input, multiline && styles.inputMultiline]} value={value} onChangeText={onChange} multiline={multiline} numberOfLines={multiline ? 3 : 1}/>
    </View>);
}
