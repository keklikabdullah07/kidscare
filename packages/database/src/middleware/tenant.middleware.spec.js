 ...
 */
async function withTenantSession(tenantId, 
...
describe('withTenantContext', () => {
    ...
    it('returns rows where tenantId matches the context', async () => {
        const extended = prisma.$extends(withTenantContext({ tenantId: TENANT_A_ID, userId: 'u', role: 'ADMIN' }));
    ...
    it('returns empty when context tenantId does not match any row', async () => {
        const extended = prisma.$extends(withTenantContext({ tenantId: 'does-not-exist-test', userId: 'u', role: 'ADMIN' }));
        const rows = await extended.user.findMany();
        expect(rows).toEqual([]);
    });
});
